...
function Import-Memberships {
    ...
    $ids = @($script:Updates.Keys)
    for ($i = 0; $i -lt $ids.Count; $i += 50) {
        $last = [Math]::Min($i + 49, $ids.Count - 1)
        $chunk = @($ids[$i..$last])
        $cond = ($chunk | ForEach-Object { "ToCIID = $_" }) -join ' OR '
        $rows = Invoke-CMWql "SELECT FromCIID, ToCIID FROM SMS_CIRelation WHERE RelationType = 1 AND ($cond)"
        foreach ($r in $rows) {
            $from = [string]$r.FromCIID
            $to = [string]$r.ToCIID
            if ($script:GroupById.ContainsKey($from) -and $script:Members.ContainsKey($to)) {
                [void]$script:Members[$to].Add($from)
            }
        }
    }
}
